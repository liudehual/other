...
{  
		int len=0;
    	adtsMutex.Lock();
		memcpy(buf,adtsFrameBuf,adtsFrameLen);
		len=adtsFrameLen;
		adtsMutex.Unlock();
        return len;  
}  
...
void * decode_audio(void *args)
{
    ...
    uint8_t* buf = (uint8_t*)av_mallocz(sizeof(uint8_t)* 1024*1024*2);  

	/* 设置回调函数 */
    pb = avio_alloc_context(buf, 1024*1024*2, 0, NULL, read_data, NULL, NULL);  
	
	pFormatCtx = avformat_alloc_context();
	pFormatCtx->pb=pb; /* 将句柄赋值给 avformat  */
#if 0
	/* 探测一下输入格式 */
...
#endif
	piFmt=av_find_input_format("aac");
	if(piFmt==NULL){
		fprintf(stderr,"Find AAC Decoder Failed \n");
		return NULL;
	}
	/* 根据指定格式打开avformat */
